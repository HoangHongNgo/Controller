...
const Put = (probs) => {
    ...
    const handlePut = (id) => {
        ...
        setIDPut(id);
        console.log("http://localhost:5000/api/games/" + idPut);
    };

    const handleOnSubmit = (e) => {
        axios
            .put("http://localhost:5000/api/games/" + idPut, {
    ...
    return (
        <div className='container'>
            ...
            <div className="d-flex justify-content-center flex-wrap">
                {probs.list.map(data =>
                    <button onClick={() => handlePut(data._id)} className={`m-3 card ${setGameClass(data.isplay)}`} key={data._id} style={{ width: '18rem', height: '20rem' }}>
